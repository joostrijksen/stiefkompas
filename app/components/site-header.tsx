  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/90 backdrop-blur">
      <div className="mx-auto max-w-7xl px-6">
        {/* 1 rij: logo | menu | actions - alles op dezelfde hoogte */}
        <div className="flex h-20 items-center justify-between gap-8">
            ...
            {MENU.map((item) =>
              item.items ? (
                ...
                >
                  <button
                    type="button"
                    className="inline-flex items-center rounded-full px-5 py-2.5 text-base font-medium text-slate-700 hover:text-slate-900"
                    aria-haspopup="menu"
                  ...
                  {open === item.label && (
                    <div
                      className="absolute left-0 pt-3 w-80"
                      role="menu"
                    >
                      <div className="rounded-2xl border border-slate-200 bg-white p-2 shadow-lg">
                        {item.items.map((sub) => (
                          <Link
                            key={sub.href}
                            href={sub.href}
                            className="group flex items-center gap-3 rounded-xl px-4 py-3 text-base text-slate-700 hover:bg-slate-50 hover:text-slate-900"
                            role="menuitem"
                          ...
                          >
                            <ChevronDown 
                              className={`h-4 w-4 shrink-0 -rotate-90 transition-transform duration-200 ${
                                hoveredItem === sub.href ? 'rotate-90' : ''
                              }`}
                            />
                            {sub.label}
                          </Link>
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link
              href="/login"
              className="hidden md:inline-flex items-center justify-center h-11 w-11 rounded-full border border-slate-200 text-slate-700 hover:bg-slate-50 hover:text-slate-900"
              aria-label="Login"
            >
        ...
        {mobileOpen && (
          <div className="md:hidden pb-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-2">
                  ...
                  {item.href ? (
                    ...
                    <>
                      <button
                        ...
                        className="w-full flex items-center justify-between rounded-xl px-4 py-3 text-base font-semibold text-slate-900 hover:bg-slate-50"
                      >
                        {item.label}
                        <ChevronDown 
                        ...
                        <div className="pl-3 mt-1">
                          {item.items?.map((s) => (
                            <Link
                              key={s.href}
                              href={s.href}
                              className="block rounded-xl px-4 py-3 text-base text-slate-700 hover:bg-slate-50"
                            >
                              {s.label}
                            </Link>
                          ))}
              ...
              <Link
                href="/login"
                className="mt-2 flex items-center justify-center gap-2 rounded-xl border border-slate-200 px-4 py-3 text-base font-semibold text-slate-900 hover:bg-slate-50"
              >
                <User className="h-5 w-5" />
                Login
              </Link>
